...
class SubscriptionsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return User.objects.filter(subscribers__user=self.request.user).distinct()
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if is_favorited in ('1', 'true', 'True') and request.user.is_authenticated:
            qs = qs.filter(in_favorites__user=request.user)
        is_in_cart = request.query_params.get('is_in_shopping_cart')
        if is_in_cart in ('1', 'true', 'True') and request.user.is_authenticated:
    ...
    def favorite(self, request, pk=None):
        ...
        return Response(RecipeMinifiedSerializer(recipe, context={'request': request}).data, status=201)
    ...
    def remove_from_cart(self, request, pk=None):
        ...
        if not deleted:
            return Response({'detail': 'Не было в списке покупок.'}, status=400)
        return Response(status=204)
